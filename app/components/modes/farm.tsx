...
export function Farm({ value, onChange }: FarmProps) {
  ...
  const applyDragMove = useCallback((x: number, y: number) => {
  ...
  const applyDragEnd = useCallback((x: number, y: number) => {
  ...
  // Desktop mouse: pointerdown works fine (not passive)
  const startDragPointer = useCallback((drag: ActiveDrag, e: React.PointerEvent) => {
    if (e.pointerType === "touch") return; // handled by native touchstart below
    e.preventDefault();
  ...
  // Mobile: native touchstart with { passive: false } so preventDefault fires before browser scrolls.
  // React's synthetic touch events are passive and cannot cancel scroll in time.
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;
    const onTouchStart = (e: TouchEvent) => {
      const target = e.target as HTMLElement;
      const dragEl = target.closest("[data-drag-kind]") as HTMLElement | null;
      if (!dragEl) return;
      const kind = dragEl.dataset.dragKind;
      const seed = dragEl.dataset.dragSeed as SeedType | undefined;
      const drag: ActiveDrag =
        kind === "water" ? { kind: "water" }
        : kind === "seed" && seed ? { kind: "seed", seed }
        : null;
      if (!drag) return;
      e.preventDefault();
      const t = e.touches[0];
  ...
  }, [mounted, applyDragMove]);

  // Global move/end so drag continues after finger leaves the element
  useEffect(() => {
    if (!mounted) return;
  ...
  }, [mounted, applyDragMove, applyDragEnd]);
  ...
  return (
    <div ref={containerRef} className="select-none space-y-3">
      ...
      <div className="flex items-center justify-between rounded-xl border border-zinc-200 bg-white px-4 py-2.5 text-sm dark:border-zinc-700 dark:bg-zinc-800">
        ...
        <div
          ...
          onPointerDown={(e) => startDragPointer({ kind: "water" }, e)}
      ...
      {/* Two-column: shop+plots on left, seed sidebar on right */}
      <div className="flex items-start gap-2">
        <div className="min-w-0 flex-1 space-y-3">
          ...
          <div className="rounded-xl border border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900/60">
            ...
            {/* Seeds */}
            <div className="p-3">
              <p className="mb-2 text-[10px] font-medium uppercase tracking-widest text-zinc-400">Seeds Shop</p>
                  ...
                  {plot.map((cell, cellIdx) => {
                    ...
                    return (
                      <div
                        ...
                        className={`relative flex h-9 w-9 cursor-pointer flex-col items-center justify-center rounded-md border transition-all ${
        ...
        {/* ── Right: seed inventory sidebar ── */}
        {hasSeedInv && (
          <div className="sticky top-2 flex flex-col items-center gap-1.5 rounded-xl border border-amber-200 bg-amber-50/80 p-1.5 dark:border-zinc-700 dark:bg-zinc-900/70">
            <span className="text-[9px] font-medium uppercase tracking-widest text-amber-400 dark:text-zinc-500">Bag</span>
            {ALL_SEEDS.filter((t) => seedInv[t] > 0).map((type) => {
              ...
              return (
                <div
                  ...
                  data-drag-kind="seed"
                  data-drag-seed={type}
                  className={`relative flex h-10 w-10 cursor-grab items-center justify-center rounded-lg border border-amber-200 bg-white text-xl shadow-sm hover:border-amber-400 active:scale-90 dark:border-zinc-600 dark:bg-zinc-800 ${
                    isDraggingThis ? "scale-90 opacity-40" : ""
                  }`}
                  onPointerDown={(e) => startDragPointer({ kind: "seed", seed: type }, e)}
                  style={{ touchAction: "none" }}
                  title={`${cfg.name} ×${seedInv[type]} — drag to plot`}
                >
                  {cfg.icon}
                  <span className="absolute -right-1 -top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-amber-400 text-[9px] font-bold leading-none text-white dark:bg-amber-500">
                    {seedInv[type]}
                  </span>
                </div>
              );
